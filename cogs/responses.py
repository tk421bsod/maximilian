...
class responses(discord.ext.commands.Cog, name='Custom Commands'):
    ...
    async def fill_cache(self):
        ...
        for guild in self.bot.guilds:
            response = await self.bot.db.exec("select * from responses where guild_id=%s", (guild.id, ))
            if not response:
                continue
            print(response)
    ...
    async def add(self, ctx, command_trigger : str, command_response : str):
        ...
        for each in self.bot.commands:
            if command_trigger.lower() == each.name.lower() or command_trigger.lower() == "jishaku" or command_trigger.lower() == "jsk":
                ...
                return
        try:
            await self.bot.db.exec("insert into responses values(%s, %s, %s)", (ctx.guild.id, command_trigger, command_response))
    ...
    async def delete(self, ctx, command_trigger : str):
        try:
            await self.bot.db.exec("delete from responses where response_trigger=%s and guild_id=%s", (command_trigger, ctx.guild.id))
            await self.fill_cache()
