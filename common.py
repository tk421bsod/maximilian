...
class db:
    ...
    def retrieve(self, database, table, valuetoretrieve, valuenametoretrieve,  retrievedvalue, debug):
        self.connect(database)
        self.dbc.execute("select {} from {} where {} like %s".format(valuetoretrieve, table, valuenametoretrieve), (retrievedvalue))
        row = self.dbc.fetchone()
        if debug == True:
    ...
    def delete(self, database, table, valuetodelete, valuenametodelete):
        ...
        if self.retrieve(database, table, valuetodelete, valuenametodelete, valuetodelete, False) == None or self.retrieve(database, table, valuetodelete, valuenametodelete, valuetodelete, False):
            ...
            return "error"
    def exec_query(self, database, querytoexecute, debug):
        self.connect(database)
